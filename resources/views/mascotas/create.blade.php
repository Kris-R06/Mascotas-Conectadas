...
<div class="w-full max-w-5xl mx-auto bg-slate-50 px-4 py-6 sm:px-6 md:p-10 font-sans">
    ...
    <div class="mb-10 border-b border-slate-200 pb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl sm:text-4xl font-extrabold text-slate-900 flex items-center gap-3">
                <i class="ph ph-plus-circle text-indigo-500"></i> Registrar Mascota
            </h1>
            <p class="mt-2 text-slate-500 font-light">
                Agrega a tu peludo al sistema para generar su placa QR o habilitar el Smart Match.
            </p>
        </div>
    ...
    <form action="{{ route('mascotas.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 sm:p-8">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="flex flex-col gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow" required>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="especie_id" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow" required>
                            ...
                            {{-- Requiere que pases la variable $especies desde el controlador --}}
                    ...
                    <div>
                        ...
                        <input type="text" name="raza" value="{{ old('raza') }}" placeholder="Ej. Mestizo" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="color" value="{{ old('color') }}" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="edad" value="{{ old('edad') }}" placeholder="Ej. 2 meses" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="estatus" class="w-full bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow" required>
                        <option value="safe" {{ old('estatus') == 'safe' ? 'selected' : '' }}>A salvo en casa</option>
                        <option value="adopcion" {{ old('estatus') == 'adopcion' ? 'selected' : '' }}>En adopción</option>
                        <option value="extraviado" {{ old('estatus') == 'extraviado' ? 'selected' : '' }}>Extraviado (Se busca)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-5">
                ...
                <div>
                    ...
                    <textarea name="descripcion" rows="3" class="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow" placeholder="Cuéntanos un poco sobre su comportamiento, si está esterilizado, etc.">{{ old('descripcion') }}</textarea>
                </div>
                ...
                <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200 mt-2">
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                        ...
                        <input type="range" name="energy_level" min="1" max="10" value="{{ old('energy_level', 5) }}" class="w-full accent-indigo-500">
                    ...
                    <div class="mt-6 flex flex-col gap-3">
                        <label class="flex items-center gap-3 cursor-pointer hover:text-indigo-600 transition-colors text-sm font-semibold text-slate-700">
                            <input type="checkbox" name="space_needed" value="1" {{ old('space_needed') ? 'checked' : '' }} class="accent-indigo-500 w-5 h-5 rounded">
                            Requiere espacio amplio / Patio
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer hover:text-indigo-600 transition-colors text-sm font-semibold text-slate-700">
                            <input type="checkbox" name="kid_friendly" value="1" {{ old('kid_friendly') ? 'checked' : '' }} class="accent-indigo-500 w-5 h-5 rounded">
        ...
        <div class="mt-10 border-t border-slate-100 pt-6 flex justify-stretch sm:justify-end">
            <button type="submit" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-10 rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2">
                <i class="ph ph-floppy-disk text-xl"></i> Guardar Mascota
            </button>
        </div>
    </form>
</div>
